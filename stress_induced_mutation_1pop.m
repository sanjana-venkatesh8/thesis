%##########################################################################
% Simplified model of stress-induced mutation rate spikes with only 1
% population.
%
...
light_exp_times = 30; %30:30:200;%[10, 30, 50, 70, 90];
...
function dxdt = population_odes(t, x)
    ...
    logistic = 1 - (eng) / P_total;

    dengdt = ((g_eng + 100 * eng2mut(t) - d_wt) * eng) * logistic * (1 - light(t)) ... % dynamics in growth restriction
           + ((g_wt - d_wt) * eng) * logistic * light(t);  % dynamics in growth permission
...
[t,y] = ode45(@population_odes,[0 200], eng0);
colors = lines(2);
...
figure;
plot(t, total_growth_rate, LineWidth=2);
title("Total growth rate vs. time")
xlabel("Time (hours)"); ylabel("Growth rate (cells/hr)")
